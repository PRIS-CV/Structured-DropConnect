...
#### Structured DropConnect Network Uncertainty for Classification
# alphas: narray, [#sample, #class], exp(outputs of the last fc layer)
# probs: narray, [#sample, #class], softmax(outputs of the last fc layer)

def sdc_differential_entropy(alphas, epsilon=1e-8):
    alphas = np.asarray(alphas, dtype=np.float64) + epsilon
    diff_entropy = np.asarray([dirichlet(alpha).entropy() for alpha in alphas])
    return diff_entropy

def sdc_max_probability(probs, epsilon=1e-8):
    max_pred_prob = np.max(probs, axis=1)
    return max_pred_prob

def sdc_uncertainty(probs,  alphas, epsilon=1e-8):

    max_prob = sdc_max_probability(probs)
    #max_mode = sdc_max_probability(mode)
    entropy_expected = dpn_entropy_expected(probs)
    diff_ent = sdc_differential_entropy(alphas)
    return max_prob, entropy_expected, diff_ent

def sdc_auc(probs, alphas, targets):
    max_prob, ent, diff_ent = sdc_uncertainty(probs, alphas)

    predictions_prob = np.argmax(probs, axis=1)
    #predictions_mode = np.argmax(np.mean(mode, axis=1), axis=1)

    rightwrong_prob = np.asarray(targets != predictions_prob, dtype=np.int32)
    #predictions_mode = np.asarray(targets != predictions_mode, dtype=np.int32)

    return (roc_auc_score(rightwrong_prob, 1. - max_prob),roc_auc_score(rightwrong_prob, ent),
            roc_auc_score(rightwrong_prob, diff_ent),
            aupr(rightwrong_prob, 1. - max_prob),  aupr(rightwrong_prob, ent),
            aupr(rightwrong_prob, diff_ent))

'''
def sdc_auc(probs, mode, alphas, targets):
    max_prob, max_mode, diff_ent = sdc_uncertainty(probs, mode, alphas)

    predictions_prob = np.argmax(np.mean(probs, axis=1), axis=1)
    predictions_mode = np.argmax(np.mean(mode, axis=1), axis=1)

    rightwrong_prob = np.asarray(targets != predictions_prob, dtype=np.int32)
    predictions_mode = np.asarray(targets != predictions_mode, dtype=np.int32)

    return (roc_auc_score(rightwrong_prob, 1. - max_prob),roc_auc_score(predictions_mode, 1. - max_mode),roc_auc_score(rightwrong_prob, diff_ent), 
           aupr(rightwrong_prob, 1. - max_prob), aupr(predictions_mode, 1. - max_mode), aupr(rightwrong_prob, diff_ent))
'''
